<template>
  <div class="container mx-auto px-4 py-8">
    ...
    <div v-else-if="!bookmarks.length" class="text-center py-8">
      ...
      <NuxtLink to="/" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded">
        Browse Recipes
      </NuxtLink>
    </div>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      >
        <img
          :src="bookmark.recipe.recipe_images[0]?.url || 'https://via.placeholder.com/400x300?text=Recipe+Image'"
        ...
        <p class="text-gray-600 mb-2 line-clamp-2">{{ bookmark.recipe.description }}</p>
        <div class="flex justify-between items-center text-sm text-gray-500 mb-2">
          <span>By: {{ bookmark.recipe.user?.username || "Anonymous" }}</span>
          <div class="flex gap-2">
            <span v-if="bookmark.recipe.prep_time_minutes" class="bg-orange-100 text-orange-800 px-2 py-1 rounded">
              {{ bookmark.recipe.prep_time_minutes }}min
            </span>
            <span v-if="bookmark.recipe.category" class="bg-blue-100 text-blue-800 px-2 py-1 rounded">
              {{ bookmark.recipe.category.name }}
            </span>
          </div>
        </div>
        <div class="flex justify-between items-center text-sm">
          <RecipeLikes :like-count="bookmark.recipe.recipe_likes_aggregate?.aggregate?.count || 0" />
